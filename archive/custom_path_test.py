...
class SimpleSimulation:
    ...
    def vehicle_random_choice(self):
        ...
        self.camera = self.world.spawn_actor(
            cam_bp, carla.Transform(loc, rot), attach_to=self.vehicle
        )
    ...
    def generate_path(self):
        ...
        pts = [
            carla.Location(x=start.x + i * 2, y=start.y + 1 * np.sin(i / 3.0), z=start.z + 0.2)
            for i in range(120)
        ]
        # Dibujamos una sola vez con mucha duración
        for i in range(len(pts) - 1):
            self.world.debug.draw_line(
                pts[i], pts[i + 1], thickness=0.1, color=carla.Color(255, 0, 0), life_time=100.0
            )
            self.world.debug.draw_point(
                pts[i], size=0.1, color=carla.Color(0, 255, 0), life_time=100.0
            )
        return pts
        ...
        try:
            while True:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            self.automated_mode = not self.automated_mode
                        if event.key == pygame.K_o:
                            self.keyboard_manual_input = False
                ...
                if self.automated_mode:
                    ...
                    control = carla.VehicleControl(
                        throttle=max(0, accel), brake=abs(min(0, accel)), steer=steer
                    )
                ...
                self.lon_history.append(e_lon)
                self.lat_history.append(e_lat)
